...
namespace Sitecore.Commerce.Plugin.Avalara.Helpers
{
    public static class SitecoreItemHelper
    {
        ...
        internal static AvalaraConfiguration GetConfiguration(CommerceContext context, IGetItemByPathPipeline getItemByPathPipeline)
        {

            var language = context.CurrentLanguage();

            var itemModelShippingPriceArgument =
                new ItemModelArgument(Constants.Tax.ConfigurationItemPath)
                {
                    Language = language
                };

            var configurationItem = getItemByPathPipeline.Run(itemModelShippingPriceArgument, context.GetPipelineContextOptions()).Result;
