...
namespace Sitecore.Commerce.Plugin.Avalara.Pipelines.Blocks
{
    public class CommitAvalaraTaxBlock : PipelineBlock<Order, Order, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<Order> Run(Order arg, CommercePipelineExecutionContext context)
        {
            ...
            var messagesItem = SitecoreItemHelper.GetAvaTaxMessages(context.CommerceContext, _getItemByPathPipeline);
            ...
            try
            {
                ...
                var commitResult = client.CommitTransaction(config.CompanyCode, transaction.code, null,null, new CommitTransactionModel() { commit = true });
                ...
                {
                    context.Logger.LogWarning("Transaction Not Logged", context);
                }

            }
            catch (Exception ex)
            {
                context.Logger.LogWarning(ex.Message, context);
            }
